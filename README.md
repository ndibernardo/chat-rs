...
```
crates/          # Shared library crates (auth, web, outbox) — no domain logic
services/        # user-service, chat-service — each hexagonal, each owns its data
contracts/proto/ # Shared gRPC contract (user-service ⇄ chat-service)
deploy/          # Kubernetes: operators, repo-owned Helm charts, kind config, Argo CD
docs/            # Architecture notes, phased implementation plans, sequence diagrams
scripts/         # Local Docker Compose tests + Kubernetes dev/staging lifecycle
keys/            # Committed dev-grade JWT keypairs (dev/staging only, never prod)
```
