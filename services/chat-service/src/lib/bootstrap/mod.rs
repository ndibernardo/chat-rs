...
pub enum Role {
    /// Everything in one process: API + WS gateway + consumers. Dev default.
    All,
    /// Channel CRUD + message history over HTTP. No Kafka consumers, no WS.
    Api,
    /// WebSocket upgrades + broadcast fan-out. Produces to Kafka; no API routes.
    Gateway,
